...
# Depending on the CLI version, "security generate" / "security id" print the
# value as a bare hex line on stdout or only as a PrvKey=/Id= log field on
# stderr. Accept both, since this script also runs against older binaries.
extract_hex() {
    local field="$1"
    sed -n "s/.*${field}=\([0-9a-f]\{64\}\).*/\1/p; /^[0-9a-f]\{64\}\$/p" | tail -n 1
}

SRV_KEY=$("$BINARY" security generate 2>&1 | extract_hex PrvKey)
SRV_ID=$("$BINARY" security id --prvkey "$SRV_KEY" 2>&1 | extract_hex Id)
COLONY_KEY=$("$BINARY" security generate 2>&1 | extract_hex PrvKey)
USER_KEY=$("$BINARY" security generate 2>&1 | extract_hex PrvKey)

if [ -z "$SRV_KEY" ] || [ -z "$SRV_ID" ] || [ -z "$COLONY_KEY" ] || [ -z "$USER_KEY" ]; then
    echo "error: could not extract keys from '$BINARY security generate/id' output" >&2
    exit 1
fi
